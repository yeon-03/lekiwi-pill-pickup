"""손목 카메라 비주얼 서보 — wrist_servo.py 이식.

원본: `/home/khw/workspace/lekiwi/yolo_and_pick/wrist_servo.py`
pick 자세 도달 이후, 손목 뷰를 보며 팔을 조금씩 뻗어 큐브를 집을 위치로 가져가는
상태기(`WristServo`)다. 게인·상태 전이는 원본과 동일하게 유지한다.
"""
...
class GraspArgs:
    ...
    # size_metric/target_size_px 를 이 파일 값으로 덮어쓴다. 없으면(빈 문자열/파일 없음) 위 기본값을 쓴다.
    ref_file: str = ""
    ...
    # pose  : pick 자세 → grasp 자세 방향으로 보간 (Step 2 자세 준비에서 저장)
    ...
    # 큐브를 물고 있는 자세 파일 (Step 2 자세 준비의 grasp_closed). 그리퍼 값만 쓴다
    close_pose_file: str = ""
    ...
    def resolve_close_pct(self) -> float:
        ...
        if self.gripper_close_pct is not None:
            ...
            if not self.close_pose_file or not path.exists():
                raise PickPlaceError(
                    f"error: 닫힘 자세 파일이 없습니다: {self.close_pose_file}\n"
                    "  워크스페이스 > 자세 준비 에서 저장하세요 (큐브를 물고 있는 상태에서 grasp_closed).\n"
                    "  또는 값을 직접: gripper_close_pct 지정\n"
                    "  닫기 단계를 끄려면: close_after_ready=false"
                )
            data = json.loads(path.read_text())
            pose = data.get("pose", data)
            if GRIPPER_JOINT not in pose:
                raise PickPlaceError(f"error: {self.close_pose_file} 에 {GRIPPER_JOINT} 가 없습니다")
    ...
    def validate(self) -> None:
        """숫자/선택지 검증만 한다. 자세 파일 존재 검사는 하지 않는다 — 워커가 자세 dict 를
        직접 주입하므로 검증 시점에는 아직 파일이 없을 수 있다 (`resolve_close_pct` 는 실제로
        그리퍼를 닫는 순간에만 파일을 읽는다)."""
        if not self.enabled:
            return
        self.load_reference()
        if self.close_after_ready:
            if self.gripper_close_time_s <= 0 or self.grasp_dwell_s < 0:
                raise PickPlaceError("error: grasp.gripper_close_time_s 는 0 보다, grasp_dwell_s 는 0 이상이어야 합니다")
        ...
        if self.max_pick_attempts < 1 or self.pick_retry_wait_s < 0:
            raise PickPlaceError("error: grasp.max_pick_attempts 는 1 이상, pick_retry_wait_s 는 0 이상이어야 합니다")
        if self.carry_time_s <= 0:
            raise PickPlaceError(f"error: grasp.carry_time_s 는 0 보다 커야 합니다 (받은 값: {self.carry_time_s})")
        if self.approach_mode not in ("pose", "joints"):
            raise PickPlaceError(f"error: grasp.approach_mode 는 pose/joints 중 하나여야 합니다 (받은 값: {self.approach_mode})")
        if self.approach_mode == "joints" and not self.reach_joints:
            raise PickPlaceError(
                "error: grasp.approach_mode=joints 에는 grasp.reach_joints 가 필요합니다 "
                "(예: {arm_shoulder_lift.pos: 5})"
            )
        if self.size_metric not in ("width", "height", "max"):
            raise PickPlaceError(f"error: grasp.size_metric 은 width/height/max 중 하나여야 합니다 (받은 값: {self.size_metric})")
        for name, v in (
            ...
            if v <= 0:
                raise PickPlaceError(f"error: grasp.{name} 은 0 보다 커야 합니다 (받은 값: {v})")
        if not 0.0 <= self.gripper_open_pct <= 100.0:
            raise PickPlaceError(f"error: grasp.gripper_open_pct 는 0~100 이어야 합니다 (받은 값: {self.gripper_open_pct})")
        ...
        if self.x_anchor not in ("left", "center", "right"):
            raise PickPlaceError(f"error: grasp.x_anchor 는 left/center/right 중 하나여야 합니다 (받은 값: {self.x_anchor})")
        if abs(self.pan_sign) != 1.0 or abs(self.tilt_sign) != 1.0:
            raise PickPlaceError("error: grasp.pan_sign / tilt_sign 은 1 또는 -1 이어야 합니다")
...
def save_reference(
    ...
    ref = {
        ...
        "source": note or "워크스페이스 Pick 실행 화면에서 [지금 화면을 참조로 저장]",
        "view": cfg.view,
        "frame_size": [w, h],
