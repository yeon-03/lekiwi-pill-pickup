"""집기 성공 판별 — grasp_check.py 이식.

원본: `/home/khw/workspace/lekiwi/yolo_and_pick/grasp_check.py`
...
class GraspCheckArgs:
    ...
    band_px: int = 40
    inset_px: int = 8
    # front 뷰의 띠 폭 배율 — 앱 추가 (2026-09-08 사용자 요청, 원본에는 없다). front 는 멀리서 보므로
    # 그리퍼 손가락이 큐브 박스 바깥으로 더 넓게 걸린다. 1.0 이면 원본과 같다. wrist 뷰는 band_px 그대로.
    ...
    def validate(self) -> None:
        ...
        if not 0 <= self.hue_min <= self.hue_max <= 180:
            raise PickPlaceError(f"error: check.hue_min/hue_max 는 0~180 이고 min<=max 여야 합니다 ({self.hue_min}, {self.hue_max})")
        if not 0 <= self.sat_min <= 255 or not 0 <= self.val_min <= 255:
            raise PickPlaceError("error: check.sat_min / val_min 은 0~255 여야 합니다")
...
def edge_strips(
    xyxy: tuple[int, int, int, int], frame_shape: tuple[int, ...], cfg: GraspCheckArgs, band_px: int | None = None,
):
    """박스 왼쪽/오른쪽 변에 걸친 띠 두 개의 (x1, y1, x2, y2). 화면 밖은 잘라낸다.

    `band_px` 를 주면 그 폭을 쓴다 (뷰별 폭 — `GraspCheckArgs.band_px_for`). 없으면 `cfg.band_px`.
    """
    h, w = frame_shape[:2]
    x1, y1, x2, y2 = xyxy
    band = cfg.band_px if band_px is None else int(band_px)
    y1c, y2c = max(0, y1), min(h, y2)
    left = (max(0, x1 - band), y1c, min(w, x1 + cfg.inset_px), y2c)
    right = (max(0, x2 - cfg.inset_px), y1c, min(w, x2 + band), y2c)
    return left, right
...
class GraspChecker:
    ...
    def _check_view(self, frame_bgr: np.ndarray, dets: list[Detection], view: str | None = None) -> ViewResult:
        res = ViewResult()
        det = largest(dets)
        if det is None:
            return res
        ...
        band = self.cfg.band_px_for(view) if view is not None else None
    ...
    def update(
        ...
        for view in (self.cfg.front_view, self.cfg.wrist_view):
            ...
            self.results[view] = (
                self._check_view(frame, dets_by_view.get(view, []), view) if frame is not None else ViewResult()
            )
